...
# integration function 1 but with no diapause

df_noDiap <- function(t, x, parms) {
  ...
  with(parms, { 
    ...
    # ODE definition 
    dE = beta*A - (h[t_n, ]*delta_E + mu_E)*E
    dJ = h[t_n, ]*delta_E*E - (delta_J + mu_J + J/K[t_n, ])*J  
    dI = 0.5*delta_J*J - (delta_I + mu_A[t_n, ])*I
    dA = delta_I*I - mu_A[t_n, ]*A
...
# this has the log of the adults, with diapause
...
# this has the log of the adults, without diapause

df_log_noDiap <- function(t, x, parms) {
  ...
  with(parms, { 
    ...
    # ODE definition 
    dlogE = beta*exp(logA)/exp(logE) - (h[t_n, ]*delta_E + mu_E)
    dlogJ = h[t_n, ]*delta_E*exp(logE)/exp(logJ) - (delta_J + mu_J + exp(logJ)/K[t_n, ])  
    dlogI = 0.5*delta_J*exp(logJ)/exp(logI) - (delta_I + mu_A[t_n, ])
    dlogA = delta_I*exp(logI)/exp(logA) - mu_A[t_n, ]
   
    dx <- c(dlogE, dlogJ, dlogI, dlogA)
